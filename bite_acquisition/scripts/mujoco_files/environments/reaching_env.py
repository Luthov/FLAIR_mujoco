...
class XArmReachEnv(ArmBaseEnv):
    ...
    | Num | Action                        | Control Min | Control Max | Unit |
    | --- | ----------------------------- | ----------- | ----------- | ---- |
    | 0   | x-coordinate of target eef    | -1          | 1           | m    |
    | 1   | y-coordinate of target eef    | -1          | 1           | m    |
    | 2   | z-coordinate of target eef    | -1          | 1           | m    |
    ...
    metadata = {
        ...
        "render_fps": 5
    }   #TODO: add functionality to render fps and add rgb_array rendering
        ...
        self.observation_space = spaces.Dict(
             {
                ...
                "target_pose": spaces.Box(low=-1.0,
                                        high=1.0,
                                        shape=(3,),
                                        dtype=np.float64),
             }
    ...
    def step(self, action):        
        ...
        if self.render_mode == "rgb_array":
            info.update({"rgb_array": pixels})
        elif self.render_mode == "depth_array":
            # TODO: should these depth values be shifted and normalized?
            # ->May not need to shift
            
            # Shift nearest values to the origin
            # pixels -= pixels.min()
            # Scale by 2 mean distances of near rays
            pixels /= 2*pixels[pixels <= 1].mean()
            # Scale to [0, 255]
            pixels = 255*np.clip(pixels, 0, 1)

            info.update({"depth_array": pixels})
    ...
    def _pre_action(self, action, policy_step=False):
        ...
        if self._robot._controller._use_ori:
            action = np.concatenate([action, np.zeros(3)])

        # Verify that the action is the correct dimension
        assert action.shape[0] == self._robot._control_dim, "Control action dimension must match robot's control dimension"

...
NOTE: convention for quaternions is (x, y, z, w) but in mujoco it is (w,x,y,z)
"""
...
def quat_multiply(quaternion1, quaternion2):
    """
    Return multiplication of two quaternions (q1 * q2).
...
def quat_distance(quat0, quat1):
    """
    Returns distance between two quaternions, such that distance * quat0 = quat1

    Args:
        quat1 (np.array): (w,x,y,z) quaternion
        quat0 (np.array): (w,x,y,z) quaternion

    Returns:
        np.array: (w,x,y,z) quaternion distance
    """
    # dtype = quat0.dtype
    # neg_quat0 = np.zeros(4, dtype=dtype)
    # err_rot_quat = np.zeros(4, dtype=dtype)

    if quat0.dot(quat1) < 0:
        quat0 = -np.array(quat0)

    # mujoco.mju_negQuat(neg_quat0, quat0)
    # mujoco.mju_mulQuat(err_rot_quat, quat1, neg_quat0)

    # print("mujoco quat distance:", err_rot_quat)
    # print("gemini quat distance:", quat_multiply(quat_inverse(quat0), quat1))
    err_rot_quat = quat_multiply(quat_inverse(quat0), quat1)
    err_rot_quat = normalize_quaternion(err_rot_quat)
    return err_rot_quat

def quat_error(quat0, quat1):
    """Returns euler distance between two quuaternion
...
def mat2quat(rmat):
    ...
    """
    q = np.zeros(4)
    mujoco.mju_mat2Quat(q, rmat.flatten())
    return q
...
def pose2mat(pose):
    ...
    Args:
        pose (2-tuple): a (pos, orn) tuple where pos is vec3 float cartesian, and
            orn is vec4 float quaternion in wxyz
...
def quat2mat(quaternion):
    ...
    """
    rmat = np.zeros(9)
    mujoco.mju_quat2Mat(rmat, quaternion)
    return rmat.reshape((3, 3))
...
def quat2axisangle(quat):
    ...
    # Calculate denomintaor for the axis computation
    den = np.sqrt(1.0 - w * w)
...
def clip_rotation_wxyz(quat, limit):
    """
    Limits a (delta) rotation to a specified limit in wxyz format.

    Converts rotation to axis-angle, clips, then re-converts back into quaternion in wxyz format.
    ...
    Returns:
        2-tuple:
            - (np.array) Clipped rotation quaternion (w, x, y, z)
            - (bool) whether the value was clipped or not
    """
    clipped = False
...
# PLEASE CHECK QUAT FORMAT BEFORE USING

...
MIN_SILENCE_DURATION_MS = 2000  # 2.0 sec silence threshold to avoid truncation
    ...
    with stream:
        ...
        while True:
            ...
            if silence_start_time is not None:
                if WAKE_WORD in " ".join(caption_cache).lower():    
                    if (time.time() - silence_start_time) > MIN_SILENCE_DURATION_MS / 1000:
                        silence_detected = True
                        print("SILENCE DETECTED! Stopping...")
                        print("caption_cache:", caption_cache)
                        ...
                        if caption_cache:  
                            user_preference = ' '.join(caption_cache).lower().split(WAKE_WORD)[1].strip()

                            # Remove leading punctuation and spaces
                            user_preference = re.sub(r"^[\s\.,!?]+", "", user_preference)
            ...
            if speech_dict:
                ...
                if "end" in speech_dict and recording:      # Speech stops
                    recording = False
                    end_recording(speech, transcribe, caption_cache)
                                
                    silence_start_time = time.time()
                    print("SPEECH STOPPED at", time.time())
